...
function RunCard({ name, tag, map50, map95, time, color, role, note }: { name: string; tag: string; map50: string; map95: string; time: string; color: string; role: string; note: string }) { return <section style={{ background: '#fff', border: '1px solid #e6eef4', borderTop: `7px solid ${color}`, borderRadius: 20, padding: '17px', boxShadow: '0 10px 28px rgba(91, 140, 174, 0.13)' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 9 }}><div style={{ color, fontSize: 29, fontWeight: 950 }}>{name}</div><div style={{ color, background: `${color}12`, border: `1px solid ${color}35`, borderRadius: 999, padding: '6px 10px', fontSize: 13, fontWeight: 900 }}>{tag}</div></div><div style={{ color, fontSize: 16, fontWeight: 950, marginBottom: 8 }}>{role}</div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 7 }}><Score label="mAP50" value={map50} color={color} /><Score label="mAP95" value={map95} color={color} /><Score label="시간" value={time} color={color} /></div><div style={{ marginTop: 10, color: '#334155', fontSize: 14.2, lineHeight: 1.34, fontWeight: 760, minHeight: 58 }}>{note}</div></section>; }
function Score({ label, value, color }: { label: string; value: string; color: string }) { return <div style={{ background: `${color}0d`, border: `1px solid ${color}25`, borderRadius: 12, padding: '9px 8px', textAlign: 'center' }}><div style={{ color: '#64748b', fontSize: 12.2, fontWeight: 900 }}>{label}</div><div style={{ color, fontSize: 16.5, fontWeight: 950, marginTop: 3 }}>{value}</div></div>; }
