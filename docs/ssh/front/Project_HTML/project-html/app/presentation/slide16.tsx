...
const roles = [
  ...
  { title: 'YOLOv8s', body: '작은 객체·원거리 탐지 비교 기준', color: '#0d9488' },
  { title: 'YOLOv11m v3', body: '위치·클래스·신뢰도 산출 주력 모델', color: '#7c3aed' },
];
const outputs = [
    ...
    <div className={styles.slide}>
      ...
      <div className={styles.contentTop} style={{ height: '100%', padding: '42px 54px 38px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, width: '100%' }}>
          ...
          <section style={card('#e11d48')}><Title color="#e11d48" text="운영 반영" /><div style={{ display: 'grid', gap: 10, marginTop: 12 }}>{outputs.map(([t,b],i) => <div key={t} style={{ display: 'grid', gridTemplateColumns: '38px 1fr', gap: 10, alignItems: 'center', background: '#f8fbfd', border: '1px solid #e2eaf0', borderRadius: 14, padding: '12px' }}><div style={{ width: 34, height: 34, borderRadius: 12, background: '#e11d48', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15.8, fontWeight: 950 }}>{i+1}</div><div><div style={{ color: '#1f2d3d', fontSize: 15.8, fontWeight: 950 }}>{t}</div><div style={{ color: '#52697f', fontSize: 13.3, lineHeight: 1.3, fontWeight: 750, marginTop: 3 }}>{b}</div></div></div>)}</div></section>
        </div>
...
function Title({ color, text }: { color: string; text: string }) { return <div style={{ color, fontSize: 18.5, fontWeight: 950 }}>{text}</div>; }
function Info({ title, body, color }: { title: string; body: string; color: string }) { return <div style={{ background: `${color}0f`, border: `1px solid ${color}30`, borderRadius: 14, padding: '10px 12px' }}><div style={{ color, fontSize: 15.8, fontWeight: 950, marginBottom: 5 }}>{title}</div><div style={{ color: '#334155', fontSize: 13.5, lineHeight: 1.3, fontWeight: 760 }}>{body}</div></div>; }
