...
**백엔드** : 라우터 · 서비스 · 스키마 · 모델 계층을 분리한 구조로 구성했습니다.
...
**AI 추론 서버**(`ai/road-ai`) : `api/v1`(엔드포인트) · `modules`(도메인 로직: keras · yolo · its) · `infrastructure`(모델 레지스트리 · 캐시 · 스토리지)로 구성해 추론 로직과 API 계층을 분리했습니다. `modules/chat`, `modules/llm`과 `api/v1/chat.py`, `api/v1/llm.py`는 코드는 존재하지만 아직 구현되지 않은 스캐폴드이며, 실제 LLM 기능은 백엔드(`chat_service.py`)와 별도의 챗봇 서버(`highway-chatbot-server`)에서 담당합니다.
