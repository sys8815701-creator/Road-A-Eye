...
본 저장소는 5인 팀 프로젝트로 개발한 Road A Eye를 개인 포트폴리오 공개 목적에 맞게 재구성한 저장소입니다. 팀 Git 이력은 포함하지 않았으며, 운영 환경의 인증정보 · 내부 네트워크 주소 · 로그 · 사용자 데이터 · 백업 파일과 AI 모델 가중치 파일(`.keras`, `.h5`, `.pt`)은 제거했습니다. 아래 디렉토리 구조와 실행 방법은 이 저장소에 실제로 포함된 코드를 기준으로 작성했습니다.
...
4개 DB(member · ai · board · chat)를 도메인별로 분리 설계하고, MySQL Master-Master 복제(binlog, auto_increment offset 분리)와 Keepalived VRRP VIP 기반 자동 페일오버(헬스체크 3초 간격)를 구성했습니다. 매일 새벽 mysqldump 기반 자동 백업(7일 보관)도 함께 운영합니다.
...
**7) 게시판 · 기타**
공지사항 · FAQ · 1:1 문의, 알림 이력, 환경설정, 마이페이지, 프로젝트 시연 프레젠테이션(슬라이드 21장 · PPTX 다운로드)을 구현했습니다.
...
프로젝트 시연 · 발표 슬라이드는 프론트엔드에 통합되어 있습니다(`frontend/project-html/app/presentation/`, 슬라이드 21장, PPTX 다운로드 지원). 별도의 시연 영상이나 DB 설계 · 발표 원본 산출물은 팀원 소유 자료로 이 저장소에는 포함되어 있지 않습니다.
