...
- AI 서버 자동 문서 : http://localhost:8001/docs
...
4개 서버로 역할을 분산했습니다. **AI 서버**는 Keras · YOLO 추론과 LLM 챗봇 연동을, **백엔드 서버**는 REST API · JWT 인증 · AI 프록시를 담당하며, **프론트 서버**는 DB에 직접 접속하지 않고 백엔드 API만 호출합니다. **DB 백업 서버**는 4개 DB(member · board · ai · chat)의 복제본을 보관하며, Keepalived 기반 VIP로 장애 시 자동 페일오버됩니다.
...
│   │   ├── app/modules/                 # keras(분류) · yolo(탐지) · llm(챗봇) · its(연동 · 앙상블 파이프라인)
...
**1) 프로젝트 기반 세팅**
FastAPI REST API 서버 · SQLAlchemy 비동기 ORM(AsyncSession) 구성, 라우터 · 서비스 · 스키마 · 모델 계층 분리, Pydantic 스키마 유효성 검사와 pydantic-settings 환경변수 관리를 갖췄습니다.

**2) 인증 · 보안**
JWT 액세스 토큰 발급 · 검증, bcrypt 비밀번호 해싱, 소셜 로그인 OAuth2(네이버 · 카카오 · 구글), 이메일 인증 코드 발송 · 검증, 로그인 시도 횟수 제한(Brute Force 방지), 역할 기반 권한(user · admin)을 구현했습니다.

**3) 사용자 관리**
회원가입 · 로그인 · 로그아웃, 소셜 계정 연동, 개인정보 조회 · 수정, 비밀번호 변경 · 재설정, 회원 탈퇴를 지원합니다.

**4) CCTV · AI 감지 연동**
CCTV 등록 · 관리, OpenCV 기반 RTSP/HTTP → MJPEG 스트리밍을 구현하고, AI 서버로부터 위험 물체 감지 결과를 수신 · 기록합니다. 감지 상태는 신규 → 확인 중 → 처리 완료로 관리하며 감지 통계 · 히트맵을 함께 제공합니다.

**5) 게시판**
공지사항 · 1:1 문의(첨부파일) · FAQ · 자료실 CRUD를 구현했습니다.

**6) 관리자 기능**
사용자 목록 조회 · 검색 · 역할 변경 · 정지 · 삭제, 활동 로그 기록 · 조회, 시스템 설정을 제공합니다.

**7) 실시간 · 부가 기능**
WebSocket 실시간 알림, ITS 외부 API 연동, FastAPI-Mail 이메일 발송, systemd 기반 서버 자동 기동을 구현했습니다.

**8) DB 이중화 · 인프라** (DB · 인프라 담당)
4개 DB(member · ai · board · chat)를 도메인별로 분리 설계하고, MySQL Master-Master 복제(binlog, auto_increment offset 분리)와 Keepalived VRRP VIP 기반 자동 페일오버(헬스체크 2초 간격)를 구성했습니다. 매일 새벽 mysqldump 기반 자동 백업(7일 보관)도 함께 운영합니다.
...
**1) 스택 세팅**
Next.js 16(App Router) · React 19 · TypeScript, CSS Modules + CSS 변수 기반 다크 · 라이트 테마를 적용했습니다.

**2) 공통 UI**
반응형 글로벌 헤더(권한별 메뉴 노출), 테마 토글, 공통 모달 시스템, AI 챗봇 플로팅 어시스턴트를 구현했습니다.

**3) 인증**
소셜 로그인(Google · Kakao · Naver) 및 콜백 처리, 로그인 상태 실시간 반영을 구현했습니다.

**4) 실시간 관제 (관리자)**
통합 관제 대시보드, 전국 CCTV 검색 후 AI 분석을 시작 · 중지하는 스트림 관리, 이미지 업로드로 여러 모델 결과를 비교하는 AI 모델 비교 화면을 구현했습니다.

**5) 분석 센터**
기간별 감지 추이 · 유형별 현황 · CCTV 순위를 보여주는 통계 리포트, 위험 구간 히트맵, 감지 기록 검토(확인 · 반려), AI 모델 관리를 구현했습니다.

**6) 관리자 시스템**
사용자 관리(권한 변경 · 정지 · 삭제), 활동 로그, 정지 문의 처리 기능을 구현했습니다.

**7) 게시판 · 기타**
공지사항 · FAQ · 1:1 문의, 알림 이력, 환경설정, 마이페이지를 구현했습니다.
...
**AI · 모델**
Keras(MobileNetV2 전이학습, 1차 분류 게이트) · YOLOv8s / YOLOv11m(Ultralytics, 위험 물체 탐지) · OpenAI API(gpt-4o, AI 관제 어시스턴트 · SSE 스트리밍)
...
**1) 분류 · 탐지 클래스 (총 10종)**
전동킥보드 · 경운기 · 굴착기 · 리어카 · 지게차 · 트랙터 · 전동휠체어 · 오토바이 · 사람 — 이상 9종이 진입 금지 대상이며, **차량(car)** 은 정상 통행 차량으로 구분되는 허용 클래스입니다 (`models/keras/v3/metadata.json` 기준).
...
Keras 분류 모델은 MobileNetV2 전이학습을 기반으로 1차 ~ 14차 반복 실험(optimizer · backbone · 해상도 비교)을 거쳐 최종 v14 모델에서 정확도 약 99%를 달성했습니다(1차 학습 기준 mAP50 97.6%).
...
최종 배포 구조는 Keras를 1차 게이트로 사용해 금지 물체 가능성을 먼저 판단한 뒤, YOLOv8s와 YOLOv11m을 함께 실행해 IoU 0.45 이상인 탐지 쌍을 가중 평균(YOLOv8s 0.35 · YOLOv11m 0.65)으로 병합하는 **3모델 앙상블(Soft Voting)** 입니다(`app/modules/its/pipeline.py`). 연산량은 줄이면서 탐지 정확도는 높이는 것을 목표로 설계했습니다.
...
### AI 서버 (`apps/ai/road-ai`)
```bash
cd apps/ai/road-ai
python -m venv venv
source venv/bin/activate
pip install -r requirements.txt
# .env 파일에서 DB 접속 정보, BACKEND_URL 등 입력
uvicorn app.main:app --host 0.0.0.0 --port 8001 --reload
```

### 프론트엔드 (`apps/front/Project_HTML/project-html`)
```bash
cd apps/front/Project_HTML/project-html
npm install
npm run dev
```

서버 최초 실행 시 SQLAlchemy `create_all`로 테이블이 자동 생성되고, 기본 관리자 계정이 시딩됩니다(`ADMIN_*` 환경변수 참조). 서버 실행 후 `/docs` 경로에서 Swagger UI로 API 명세를 확인할 수 있습니다.

---

## 환경변수

`.env` 파일은 보안상 저장소에 포함되지 않으며, 각 서비스의 `env.example`을 참고해 로컬에 직접 생성해야 합니다.

**백엔드 (`apps/back/FastAPI/.env`)**
...
**AI 서버 (`apps/ai/road-ai/.env`)**
`AI_API_KEY` · `DB_HOST` · `DB_USER` · `DB_PASSWORD` · `DB_NAME`(ai_db) · `BACKEND_URL`
...
- **김사브리나** — 프론트엔드, Next.js 대시보드 33개 페이지 구현
- **심유경** — 백엔드, FastAPI 8개 라우터 · 4개 DB 연동
...
AI 서버는 `api/v1`(엔드포인트) · `modules`(도메인 로직: keras · yolo · llm · its) · `infrastructure`(모델 레지스트리 · 캐시 · 스토리지)로 구성해 추론 로직과 API 계층을 분리했습니다.

---

## 시연 자료

발표 자료 및 시연 영상은 `db/presentation/` 경로에 포함되어 있습니다.
