  ...
  return (
    <div className={styles.slide}>
      ...
      <div className={styles.contentTop} style={{ padding: '70px 60px 40px' }}>
        ...
        <p className={styles.slideSubtitle} style={{ marginBottom: 30 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 26, width: '100%', padding: '0 20px', boxSizing: 'border-box', alignItems: 'stretch' }}>
          ...
          <div style={{
            ...
            {bgGroups.map((g) => (
              <div key={g.title}>
                <div style={{ fontSize: 17, fontWeight: 800, color: '#1f2d3d', marginBottom: 12, letterSpacing: '-0.2px' }}>{g.title}</div>
                <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 9 }}>
                  {g.items.map((it, i) => (
                    <li key={i} style={{ display: 'flex', gap: 10, fontSize: 14, color: '#46586a', lineHeight: 1.5 }}>
          ...
          <div style={{
              ...
              {goalItems.map((it, i) => (
                <div key={i} style={{
                  ...
                  <span style={{
                    ...
                    background: GOAL, color: '#fff', fontSize: 14, fontWeight: 800,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                  }}>{i + 1}</span>
                  <span style={{ fontSize: 16, fontWeight: 700, color: '#1f2d3d', lineHeight: 1.3 }}>{it}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{
              ...
              padding: '14px 18px', fontSize: 15, color: '#33475a', fontWeight: 600, lineHeight: 1.5,
            }}>
