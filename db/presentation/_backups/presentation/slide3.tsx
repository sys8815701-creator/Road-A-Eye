...
export default function Chapter1() {
  ...
  const members: Member[] = [
    ...
    { name: '이지건', role: 'Keras-AI', img: '/members/이지건이모지.png', accent: '#2e7d8a', tags: ['Keras', 'TensorFlow', 'Deep Learning'], desc: '딥러닝 분류 모델 · mAP50 97.6%', detail: aiModelDetail, detailTitle: 'AI 모델 작업 내역' },
        ...
        <div style={{
          ...
          {members.map((m) => (
            ...
            >
              ...
              <div style={{ marginTop: 16, fontSize: 22, fontWeight: 800, color: '#1f2d3d', letterSpacing: '-0.3px' }}>{m.name}</div>

              <div style={{
                marginTop: 9, background: m.accent, color: '#fff', fontSize: 14, fontWeight: 700,
                padding: '5px 16px', borderRadius: 20, letterSpacing: '0.2px', whiteSpace: 'nowrap',
              }}>{m.role}</div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 6, marginTop: 16 }}>
                {m.tags.map((t) => (
                  <span key={t} style={{
                    ...
                    padding: '4px 11px', borderRadius: 7, fontSize: 14, fontWeight: 600,
                  }}>{t}</span>
                ))}
              </div>
              ...
              {m.detail && (
                <button
                  ...
                  style={{
                    ...
                    padding: '12px 14px', fontSize: 14.5, fontWeight: 800, cursor: 'pointer',
      ...
      {active && (
        ...
        >
          ...
          >
            ...
            {/* 본문 — 3단 마소너리 */}
            <div style={{
              flex: 1, overflowY: 'auto', padding: '22px 28px 28px',
              ...
              {active.detail!.map((c, i) => (
                <div key={c.title + i} style={{
                  ...
                  padding: '14px 16px 15px', marginBottom: 16,
                }}>
                  <div style={{
                    display: 'flex', alignItems: 'center', gap: 9, marginBottom: 11,
                    paddingBottom: 9, borderBottom: '1px solid #eef2f6',
                  }}>
                    <span style={{
                      flexShrink: 0, fontSize: 12, fontWeight: 800, color: '#fff',
                      background: active.accent, borderRadius: 6, padding: '3px 8px', letterSpacing: '0.5px',
                    }}>{c.badge}</span>
                    <span style={{ fontSize: 16, fontWeight: 800, color: '#28323d', lineHeight: 1.25 }}>{c.title}</span>
                    {c.group && (
                      <span style={{
                        marginLeft: 'auto', fontSize: 11, fontWeight: 700, color: active.accent,
                        background: `${active.accent}14`, border: `1px solid ${active.accent}33`, borderRadius: 10, padding: '2px 8px',
                        whiteSpace: 'nowrap',
                      }}>{c.group}</span>
                    )}
                  </div>
                  <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {c.items.map((it, j) => (
                      <li key={j} style={{ display: 'flex', gap: 8, fontSize: 13.5, color: '#46586a', lineHeight: 1.5 }}>
                        {c.numbered ? (
                          <span style={{
                            flexShrink: 0, minWidth: 17, fontSize: 12.5, fontWeight: 800,
                            color: active.accent, textAlign: 'right',
                          }}>{j + 1}.</span>
                        ) : (
                          <span style={{ flexShrink: 0, width: 5, height: 5, borderRadius: '50%', background: active.accent, marginTop: 7 }} />
                        )}
                        <span>{it}</span>
                      </li>
                    ))}
