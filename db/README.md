...
| 폴더 | 내용 |
|------|------|
| [presentation/](./presentation/) | 발표 슬라이드쇼 앱 (Next.js, 21장) + 데모 영상 |

> `node_modules`·`.next` 빌드 산출물은 제외돼 있습니다. 실행은 `presentation/` 에서 `npm install` 후 빌드하세요.
...
- 실데이터 백업은 db 서버(<DB_SERVER_IP>)의 `~/db_backups/` 에 매일 자동 보관됩니다.
