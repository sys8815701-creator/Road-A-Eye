...
const items = [
  ...
  { icon: '👤', title: '회원 시스템', points: ['가입(이메일 인증) · 로그인', 'JWT 8시간 토큰 인증', '프로필 수정 · 탈퇴'] },
...
function Feature({ icon, title, points }: { icon: string; title: string; points: string[] }) {
  return (
    <div className={styles.featureCard} style={{ padding: '24px 24px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 13, marginBottom: 18 }}>
        <div className={styles.featureIcon} style={{ width: 50, height: 50, fontSize: 28, flexShrink: 0 }}>{icon}</div>
        <div className={styles.featureTitle} style={{ fontSize: 23, lineHeight: 1.18, marginBottom: 0 }}>{title}</div>
      </div>
      <ul className={styles.bulletList} style={{ marginTop: 0 }}>
        {points.map((p, idx) => <li key={p} style={{ fontSize: 18.6, lineHeight: 1.42, marginBottom: idx === points.length - 1 ? 0 : 10 }}>{p}</li>)}
      </ul>
    </div>
  );
}
